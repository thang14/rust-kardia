...
use kp_core::{
    storage::{well_known_keys::is_child_storage_key, ChildInfo, StateVersion, TrackedStorageKey},
    H160, H256, U256,
};
...
use kp_types::{account::Account, log::Log};
    ...
    B: 'a + Backend<H>,
{
    ...
    // the cache account
    accounts_cache: BTreeMap<H160, Account>,

    // transaction logs
    logs: BTreeMap<H256, Vec<Log>>,
    ...
    B: Backend<H>,
{
    ...
    pub fn new(
        ...
        Self {
            overlay,
            backend,
            accounts_cache: BTreeMap::default(),
    ...
    B: Backend<H>,
{
    ...
    fn storage_commit_transaction(&mut self) -> Result<(), ()> {
        // set account storage
        for ac in self.accounts_cache {
            self.overlay.set_storage(ac.0, Some(ac.1.to_rlp()));
        }
...
where
    ...
    B: Backend<H>,
{
    fn get_account(&mut self, address: H160) -> Option<Account> {
        if self.accounts_cache.contains_key(&address) {
            return self.accounts_cache.get(&address);
        }

        let res = self
            .storage(address.as_bytes())
            .as_ref()
            .map(|x| Account::from_rlp(x).expect("decode error"));

        self.accounts_cache.insert(&address, res);

        res
    }

    fn get_account_nonce(&self, address: H160) -> Option<&U256> {
        self.get_account(address).map(|a| a.nonce())
    }

    fn inc_nonce(&self, address: H160) {
        self.get_account(address).map(|a| a.inc_nonce());
    }

    fn set_code(&self, address: H160, code: H256) {
        self.get_account(address).map(|a| a.set_code(code));
    }


    fn transfer(&self, from: H160, to: H160, amount: U256) {
        self.get_account(from).map(|a| a.sub_balance(&amount));
        self.get_account(to).map(|a| a.add_balance(&amount));
    }

    fn add_log(&self, tx_hash: H256, log: Log) {
        self.logs.insert(tx_hash, log)
    }
}
